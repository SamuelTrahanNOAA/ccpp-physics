...
!                         firesize,mean_fct,                         &
  ...
  INTEGER, PARAMETER ::  imean_frp=1, istd_frp=2!, imean_fsize=3, istd_fsize=4       ! RAR:

!  integer, intent(in) :: PLUMERISE_flag
  real(kind=kind_phys), dimension(2)  :: plume_fre
...
!               FRP = 1000.  ! 05/11/2021 for testing purpose
    ...
    enddo lp_minmax
        ...
        !dzi= 1./(zzcon(k2)-zzcon(k1))   ! RAR: k2>=k1+1  
...
!    integer k_initial,k_final,ko,kk4,kl
    ...
    k2= MIN(31,k_lim(2))   ! RAR: the model doesn't simulate very high injection heights, so it's safe to assume maximum heigh of 12km AGL for HRRR grid
