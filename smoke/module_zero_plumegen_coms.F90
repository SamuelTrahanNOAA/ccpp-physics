...
!
real(kind=kind_phys),dimension(nkp) ::  pke,the,thve,thee,pe,te,qvenv,rhe,dne,sce ! environment at plume grid
real(kind=kind_phys),dimension(nkp) ::  ucon,vcon,wcon,thtcon ,rvcon,picon,tmpcon,dncon,prcon &
...
integer :: N,NM1,L
!
real(kind=kind_phys) :: ADVW,ADVT,ADVV,ADVC,ADVH,ADVI,CVH(nkp),CVI(nkp),ADIABAT,&
        WBAR,ALAST(10),VHREL,VIREL  ! advection
!
real(kind=kind_phys) :: ZSURF,ZBASE,ZTOP
integer :: LBASE
!
