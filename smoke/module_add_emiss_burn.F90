...
CONTAINS
  subroutine add_emis_burn(data,dtstep,ktau,dz8w,rho_phy,rel_hum,      &
                           ...
                           r_q,fhist,aod3d,                          &
 !                         nwfa,nifa,                                &
                           rainc,rainnc, swdown,do_forecast,         &
...
!   TYPE(grid_config_rec_type),  INTENT(IN   )    :: config_flags
   ...
   real(kind_phys), DIMENSION(ims:ime,kms:kme,jms:jme), INTENT(OUT) ::  aod3d    ! RAR:
...
!             if (icall<5000 .AND. config_flags%debug_chem) then
             if (icall<5000 .and. dbg_opt) then
               if ( k==kts ) then
