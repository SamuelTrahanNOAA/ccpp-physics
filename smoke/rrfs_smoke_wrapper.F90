...
!> @{
    subroutine rrfs_smoke_wrapper_run(im, kte, kme, ktau, dt, garea, land, jdate,      &
                   ...
                   dust12m_in, smoke_GBBEPx, ntrac, qgrs, gq0, tile_num,               &
                   ntsmoke, ntdust, imp_physics, imp_physics_thompson,                 &
                   emdust, emseas, ebb_smoke_hr, frp_avg_hr, frp_std_hr,               &
                   coef_bb, ebu_smoke,fhist, min_fplume, max_fplume, hwp,              &
                   seas_opt_in, dust_opt_in, biomass_burn_opt_in, drydep_opt_in,       &
                   plumerise_flag_in,addsmoke_flag_in,                                 &
                   do_forecast_in,dbg_opt_in,errmsg,errflg)
    ...
    logical,        intent(in) :: do_forecast_in,dbg_opt_in
   ...
   !real(kind_phys), dimension(im,   10), intent(in) :: emi_in
    ...
    real(kind_phys), dimension(im), intent(inout) :: emdust, emseas
    real(kind_phys), dimension(im), intent(inout) :: ebb_smoke_hr, frp_avg_hr, frp_std_hr
    ...
    integer,        intent(in) :: seas_opt_in, dust_opt_in, biomass_burn_opt_in,  &
                                  drydep_opt_in, plumerise_flag_in, addsmoke_flag_in
...
!>- plume variables
    ...
    real(kind_phys), dimension(ims:im, jms:jme,2) :: plume_fre
    real(kind_phys), dimension(ims:im, jms:jme )  :: coef_bb_dc, flam_frac,             &
                     fire_hist, peak_hr
    real(kind_phys), dimension(ims:im,kms:kme,jms:jme ) ::   aod3d_smoke
...
!>-- options to turn on/off sea-salt, dust, plume-rising
    ...
    !-- temporally turn off the plume, only use BB for surface layer
    !plumerisefire_frq = 0
    ! -- set control flags
...
!>- get ready for chemistry run
    call rrfs_smoke_prep(                                               &
       ...
       !emis_ant,                                                       &
        ...
        ebb_smoke_hr, frp_avg_hr, frp_std_hr,                           &
...
!END IF
     ...
     do j=jts,jte
      do i=its,ite
        plume_fre(i,j,1)= 1.e+6*coef_bb_dc(i,j)* plume_frp(i,j,p_frp_mean     )
        plume_fre(i,j,2)= 1.e+6*coef_bb_dc(i,j)* plume_frp(i,j,p_frp_std      )
       !plume_fre(i,j,3)= 1.e+6*coef_bb_dc(i,j)* plume_frp(i,j,p_frp_mean_size)
       !plume_fre(i,j,4)= 1.e+6*coef_bb_dc(i,j)* plume_frp(i,j,p_frp_std_size )
      enddo
     enddo
...
!>- compute sea-salt
    ...
    if (call_plume) then
        call ebu_driver (                                              &
                   ...
                   z_at_w,zmid,ktau,                                   &
                   plume_fre, min_fplume2, max_fplume2,                &   ! new approach
                   ids,ide, jds,jde, kds,kde,                          &
                   ims,ime, jms,jme, kms,kme,                          &
                   its,ite, jts,jte, kts,kte                           )
    end if
    ...
    call add_emis_burn(dtstep,ktau,dz8w,rho_phy,rel_hum,chem,        &
...
!
 ...
 subroutine rrfs_smoke_prep(                                            &
        ...
        pr3d,ph3d,phl3d,tk3d,prl3d,us3d,vs3d,spechum,exch,w,            &
        nsoil,smc,vegtype,soiltyp,sigmaf,dswsfc,zorl,                   &
        snow_cpl,dust12m_in,smoke_GBBEPx,                               &
        hf2d, pb2d,                                                     &
        u10,v10,ust,tsk,xland,xlat,xlong,dxy,                           &
        rri,t_phy,u_phy,v_phy,p_phy,rho_phy,dz8w,p8w,                   &
        t8w,exch_h,                                                     &
   ...
   !real(kind_phys), dimension(ims:ime, kms:kemit, jms:jme, num_emis_ant), intent(inout) :: emis_ant
    ...
    real(kind_phys), dimension(ims:ime), intent(inout) :: ebb_smoke_hr, frp_avg_hr, frp_std_hr
   !real(kind_phys), dimension(ims:ime, jms:jme) :: emiss_abu
   ...
   !real(kind_phys), parameter :: frpc  = 1.e+09_kind_phys      ! FRP conversion factor (Global)
    ...
    select case (plumerise_flag)
      case (FIRE_OPT_GBBEPx)
        do j=jts,jte
         do i=its,ite
          plume   (i,j,1)  =smoke_GBBEPx(i,current_hour,2) ! mean frp
          ebb_smoke_hr(i)  =smoke_GBBEPx(i,current_hour,1) ! smoke
          frp_avg_hr  (i)  =smoke_GBBEPx(i,current_hour,2) ! mean frp
          frp_std_hr  (i)  =smoke_GBBEPx(i,current_hour,3) ! std frp
          ebu_in    (i,j)  = ebb_smoke_hr(i)
          plume_frp(i,j,p_frp_mean     ) = frp_avg_hr  (i) 
          plume_frp(i,j,p_frp_std      ) = frp_std_hr  (i)
         !plume_frp(i,j,p_frp_mean_size) = msize(ivgtyp(i,j)) * plume(i,j,1)
         !plume_frp(i,j,p_frp_std_size ) = 0.5_kind_phys * plume_frp(i,j,p_frp_mean_size)
         enddo
        enddo
      case default
