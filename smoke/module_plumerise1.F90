...
!   INTEGER, PARAMETER ::  iflam_frac  =1
   REAL, PARAMETER :: frp_threshold= 1.e+7   ! Minimum FRP (Watts) to have plume rise 
    
   real(kind=kind_phys), DIMENSION( ims:ime, jms:jme, 2 ), INTENT(IN ) :: plume_fre          ! RAR: FRP etc. array
...
!          enddo
        ...
        IF (icall<2000 .and. dbg_opt) then
...
!    IF (ktau==2) THEN
       do j=jts,jte
        do i=its,ite
           flam_frac(i,j)= 0.
           if (plume_fre(i,j,1) > frp_threshold) then 
              flam_frac(i,j)= 0.9
           end if
        enddo
       enddo
...
check_pl:  IF (plumerise_flag == 2 ) THEN    ! if the namelist option is set for plumerise 
       do j=jts,jte
          do i=its,ite
               k_min(i,j)=0
               k_max(i,j)=0

   !            if (plume_fre(i,j,1) < frp_threshold ) cycle  ! FRP is too small, 10 MW
               !do nv=2,num_ebu
