...
contains
  ...
  real(kind_phys), DIMENSION (1,1,1) :: tmp,airden,airmas,oh,xno3,h2o2,chldms_oh,    &
                               chldms_no3,chldms_x,chpso2,chpmsa,chpso4,    &
                               chlso2_oh,chlso2_aq,cldf
  real(kind_phys), DIMENSION (1,1,4) :: tdry
  real(kind_phys), DIMENSION (1,1) :: cossza
...
! USE module_data_gocart_chem
  ...
  REAL(kind_phys), PARAMETER :: b = 0.25
  
  ! From D4: only 0.8 efficiency, also some goes to DMSO and lost. 
  ! So we assume 0.75 efficiency for DMS addtion channel to form    
  ! products.                                                       
  
  REAL(kind_phys), PARAMETER :: eff = 1.0
  ! -- Factor to convert AIRDEN from kgair/m3 to molecules/cm3: 
  REAL(kind_phys), PARAMETER :: f = 1000.0 / airmw * 6.022D23 * 1.0D-6
  INTEGER :: i, j, l
  REAL(kind_phys) :: tk, o2, dms0, rk1, rk2, rk3, dms_oh, dms, xoh, xn3, xx
...
!  REAL(kind_phys), DIMENSION(:,:),     INTENT(INOUT) :: depso2
  ...
  ! Factor to convert AIRDEN from kgair/m3 to molecules/cm3: 
  REAL(kind_phys), PARAMETER :: f  = 1000. / airmw * 6.022D23 * 1.0D-6
  REAL(kind_phys), PARAMETER :: ki = 1.5D-12
  INTEGER :: i, j, l
  REAL(kind_phys) :: so20, tk, f1, rk1, rk2, rk, rkt, so2_cd, fc, so2
