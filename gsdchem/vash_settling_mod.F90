...
SUBROUTINE vash_settling_driver(dt,t_phy,moist,                            &
  ...
  real(kind_phys), DIMENSION (1,1,kte-kts+1,5) :: dust
  real(kind_phys), DIMENSION (1,1,kte-kts+1,4) :: sea_salt
!srf
  real(kind_phys), DIMENSION (1,1,kte-kts+1,10) :: ash
  real(kind_phys), DIMENSION (10), PARAMETER :: den_ash(10)=(/2500.,2500.,2500.,2500.,2500., &
                                                     2500.,2500.,2500.,2500.,2500. /)
  real(kind_phys), DIMENSION (10), PARAMETER :: reff_ash(10)=(/0.5000D-3,&! 1.00 mm diameter 
                                                      0.3750D-3,&! 0.75 mm
                              0.1875D-3,&!
                              93.750D-6,&!
                              46.875D-6,&!
...
!srf
  real(kind_phys), DIMENSION (1,1,kte-kts+1,10) :: ash 
  real(kind_phys), DIMENSION (4), PARAMETER :: den_ash(4)=(/2500.,2500.,2500.,2500. /)
  real(kind_phys), DIMENSION (4), PARAMETER :: reff_ash(4)=(/ 11.719D-6,&!
                              05.859D-6,&!
                              02.930D-6,&!
                              00.975D-6 /)! 3.9 um
...
! **************************************************************************** 
  ...
  REAL(kind_phys)    :: dzmin, vsettl, dtmax, pres, rhb, rwet(nmx), ratio_r(nmx)
  REAL(kind_phys)    :: addmassf,c_stokes, free_path, c_cun, viscosity, vd_cor, growth_fac,mass_above
  REAL(kind_phys),    PARAMETER :: dyn_visc = 1.5E-5
  INTEGER :: iprt,k, n, i, j, l, l2
  ! for sea-salt:
  REAL(kind_phys), PARAMETER :: c1=0.7674, c2=3.079, c3=2.573E-11, c4=-1.424 

  ! for OMP:
  REAL(kind_phys) :: rwet_priv(nmx), rho_priv(nmx),vsettl_max(nmx)
