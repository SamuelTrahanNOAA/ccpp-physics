...
!************************************************************************
        ...
        real(kind_phys), parameter :: hygro_msa_aer = 0.58
        integer nspint
...
!
   ...
   real(kind_phys), dimension( 1:nbin_o, kts:kte) ::                              &
   ...
   real(kind_phys), dimension( kts:kte ) :: dz
!
   integer ns,i,j,iclm, jclm, k, isize
   integer option_method, option_mie
   real(kind_phys), dimension( nspint, kts:kte ) ::                               &
           swsizeaer,swextaer,swwaer,swgaer,swtauaer,swbscoef
   real(kind_phys), dimension( 16 , kts:kte ) ::                               &
           lwtauaer,lwextaer
   real(kind_phys), dimension( nspint, kts:kte ) ::                               &
           l2, l3, l4, l5, l6, l7
   real(kind_phys) refr
   real(kind_phys) fv,vsum
...
!
   ...
   real(kind_phys)  dens_so4  , dens_no3  , dens_cl   , dens_msa  , dens_co3 ,  &
         ...
         dens_dust
   real(kind_phys)  mass_so4  , mass_no3  , mass_cl   , mass_msa  , mass_co3 ,  &
         ...
         mass_dust
   real(kind_phys)  mass_so4i , mass_no3i , mass_cli  , mass_msai , mass_co3i,  &
   ...
   real(kind_phys)  mass_so4j , mass_no3j , mass_clj  , mass_msaj , mass_co3j,  &
   ...
   real(kind_phys)  mass_antha, mass_seas, mass_soil
   real(kind_phys)  vol_so4   , vol_no3   , vol_cl    , vol_msa   , vol_co3  ,  &
         ...
         vol_dust
   real(kind_phys)  conv1a, conv1b, conv1sulf
   real(kind_phys)  mass_dry_a, mass_wet_a, vol_dry_a , vol_wet_a , vol_shell,  &
         dp_dry_a  , dp_wet_a  , num_a     , dp_bc_a
   real(kind_phys)  ifac, jfac, cfac
   integer ns
   real(kind_phys)  dgnum_um,drydens,duma,dlo_um,dhi_um,dgmin,sixpi,ss1,ss2,ss3,dtemp
   integer  iflag
...
! 7/21/09  SAM variables needed to convert GOCART sectional dust and seasalt to MOZAIC sections
   ...
   real(kind_phys), save :: seasfrc_goc8bin(number_ss_bins,nbin_omoz)   ! GOCART seasalt size distirbution - mass fracs in MOSAIC 8-bins
   real(kind_phys), save :: dustfrc_goc8bin(ndust,nbin_omoz)   ! GOCART dust size distribution - mass fracs in MOSAIC 8-bins
   real(kind_phys), save :: ashfrc_goc8bin(4,nbin_omoz)   ! ash size distibution
   real(kind_phys)  mass_bc1 , mass_bc2 , vol_bc2  , mass_bc1j , mass_bc2j,  &
         mass_bc1i , mass_bc2i  , vol_soil
   real(kind_phys) :: dlogoc, dhigoc
   integer istop,pdust
   integer, save :: kcall
   data  kcall / 0 /

!
!  real(kind_phys)  sginin,sginia,sginic from module_data_sorgam.F
! 
...
!         mass_ba1j , mass_ba2j,  mass_ba3j , mass_ba4j , mass_paj,   &
         ...
         mass_h2oj , mass_dustj
   real(kind_phys)  mass_antha, mass_seas, mass_soil
   real(kind_phys)  num_ai, num_aj, num_ac,  vol_ai, vol_aj, vol_ac
   real(kind_phys)  vol_so4   , vol_no3   , vol_cl    , vol_msa   , vol_co3  ,  &
         vol_nh4   , vol_na    , vol_ca    , vol_oin   , vol_oc   ,  &
!         vol_bc    , vol_aro1  , vol_aro2  , vol_alk1  , vol_ole1 ,  &
!         vol_api1  , vol_api2  , vol_lim1  , vol_lim2  , vol_h2o  ,  &
         vol_bc    , vol_h2o   , vol_dust
   real(kind_phys)  conv1a, conv1b
   real(kind_phys)  mass_dry_a, mass_wet_a, vol_dry_a , vol_wet_a , vol_shell,  &
         dp_dry_a  , dp_wet_a  , num_a     , dp_bc_a
   real(kind_phys)  ifac, jfac, cfac
   real(kind_phys)  refr
...
!       integer,parameter :: nspint = 4 ! Num of spectral for FAST-J
        ...
        real(kind_phys) p2(nsiz),p3(nsiz),p4(nsiz),p5(nsiz)
        real(kind_phys) p6(nsiz),p7(nsiz)
        logical perfct,anyang,prnt(2)
        real(kind_phys) xmu(1)
        data xmu/1./,anyang/.false./
        data prnt/.false.,.false./
        integer numang,nmom,ipolzn,momdim
        data numang/0/
        complex s1(1),s2(1)
        real(kind_phys) mimcut
        data perfct/.false./,mimcut/0.0/
        data nmom/7/,ipolzn/0/,momdim/7/
        integer n
        real(kind_phys) thesize    ! 2 pi radpart / waveleng = size parameter
        real(kind_phys) qext(nsiz) ! array of extinction efficiencies
        real(kind_phys) qsca(nsiz) ! array of scattering efficiencies
        real(kind_phys) gqsc(nsiz) ! array of asymmetry factor * scattering efficiency
        real(kind_phys) qext4(nsiz)          !  extinction, real*4
        real(kind_phys) qsca4(nsiz)          !  extinction, real*4
        real(kind_phys) qabs4(nsiz)          !  extinction, real*4
        real(kind_phys) asymm(nsiz)  ! array of asymmetry factor
        real(kind_phys) sb2(nsiz)     ! JCB 2007/02/01 - 4*abs(sback)^2/(size parameter)^2 backscattering efficiency
        complex crefin,crefd,crefw
        save crefw
        real(kind_phys), save :: rmin,rmax   ! min, max aerosol size bin
        real(kind_phys) bma,bpa
        real(kind_phys) refr     ! real part of refractive index
        real(kind_phys) refi     ! imaginary part of refractive index
        real(kind_phys) refrmin ! minimum of real part of refractive index
        real(kind_phys) refrmax ! maximum of real part of refractive index
        real(kind_phys) refimin ! minimum of imag part of refractive index
        real(kind_phys) refimax ! maximum of imag part of refractive index
        real(kind_phys) drefr ! increment in real part of refractive index
        real(kind_phys) drefi ! increment in imag part of refractive index
        complex specrefndx(ltype) ! refractivr indices
        integer, parameter ::  naerosols=5

        !parameterization variables
        real(kind_phys) weighte, weights,weighta
        real(kind_phys) x
        real(kind_phys) thesum ! for normalizing things
        real(kind_phys) sizem ! size in microns
        integer m, j, nc, klevel
        real(kind_phys) pext           ! parameterized specific extinction (cm2/g)
        real(kind_phys) pscat      !scattering cross section
        real(kind_phys) pabs           ! parameterized specific extinction (cm2/g)
        real(kind_phys) pasm       ! parameterized asymmetry factor
        real(kind_phys) ppmom2     ! 2 Lengendre expansion coefficient (numbered 0,1,2,...)
        real(kind_phys) ppmom3     ! 3     ...
        real(kind_phys) ppmom4     ! 4     ...
        real(kind_phys) ppmom5     ! 5     ...
        real(kind_phys) ppmom6     ! 6     ...
        real(kind_phys) ppmom7     ! 7     ...
        real(kind_phys) sback2     ! JCB 2007/02/01 sback*conjg(sback)
        real(kind_phys) cext(ncoef),casm(ncoef),cpmom2(ncoef),cabs(ncoef)
        real(kind_phys) cscat(ncoef)  ! JCB 2004/02/09
        real(kind_phys) cpmom3(ncoef),cpmom4(ncoef),cpmom5(ncoef)
        real(kind_phys) cpmom6(ncoef),cpmom7(ncoef)
        real(kind_phys) cpsback2p(ncoef) ! JCB 2007/02/09  - backscatter
        integer itab,jtab
        real(kind_phys) ttab,utab
        real(kind_phys), save :: xrmin,xrmax,xr
        real(kind_phys) rs(nsiz) ! surface mode radius (cm)
        real(kind_phys) xrad ! normalized aerosol radius
        real(kind_phys) ch(ncoef) ! chebychev polynomial


        !others
        integer i,k,l,ns  
        real(kind_phys) pie,third
        integer ibin
        character*150 msg
...
!     See Numerical Recipes, pp. 148-150.
      ...
      real(kind_phys) c(ncoef),f(n)

! local variables      
      real(kind_phys) fac, thesum
      integer j, k
...
!
      implicit none
      integer im,jm,km
      real(kind_phys) table(km,im,jm),xtab(im),ytab(jm),out(km)
      integer i,ix,ip1,j,jy,jp1,k
      real(kind_phys) x,dx,t,y,dy,u,tu,  tuc,tcu,tcuc
...
! ----------------------------------------------------------------------
      ...
      real(kind_phys)     gqsc, mimcut, pmom( 0:momdim, * ), qext, qsca,   &
               xmu(*), xx
      complex  crefin, sforw, sback, s1(*), s2(*), tforw(*),   &
               tback(*)
      integer maxang,mxang2,maxtrm
      real(kind_phys) onethr
! ----------------------------------------------------------------------
!
...
!
      ...
      real(kind_phys)      mim, mimsav, mre, mm, np1dn
      real(kind_phys) rioriv,xmusav,xxsav,sq,fn,rn,twonp1,tcoef, coeff
      real(kind_phys) xinv,psinm1,chinm1,psin,chin,rtmp,taun
      real(kind_phys)      rbiga( maxtrm ), pin( maxang ), pinm1( maxang )
...
!
      ...
      parameter  ( maxtrm = 1102, maxmom = 2*maxtrm, mxmom2 = maxmom/2,   &
                   maxrcp = 4*maxtrm + 2 )
      real(kind_phys)      am( 0:maxtrm ), bi( 0:mxmom2 ), bidel( 0:mxmom2 )
      real(kind_phys), save :: recip( maxrcp )
      complex cm( maxtrm ), dm( maxtrm ), cs( maxtrm ), ds( maxtrm ),   &
                 c( maxtrm ), d( maxtrm )
      integer k,j,l,nummom,ld2,idel,m,i,mmax,imax
      real(kind_phys) thesum
...
!
      ...
      real(kind_phys)    pmom( 0:momdim, * ),sq,a1sq,b1sq
...
!
      ...
      real(kind_phys)    pmom( 0:momdim, * ),sq,pm1,pm2,a2sq,b2sq
      complex  a(*), b(*)
...
!
      ...
      integer  ntrm,n
      real(kind_phys)    mre, mim, rbiga(*), xx, rezinv, rtmp, f1,f2,f3
...
!
      ...
      real(kind_phys)    gqsc, pmom( 0:momdim, * ), qext, qsca, xx, xmu(*)
      real(kind_phys) fi1,fi2,fnorm
...
!
      implicit none
      integer  numang,j
      real(kind_phys)    gqsc, qext, qsca, xx, xmu(*)
      real(kind_phys) twothr,fivthr,fivnin,sq,rtmp
...
!
      ...
      integer  numang,j
      real(kind_phys)    gqsc, qext, qsca, xx, xmu(*)
      real(kind_phys) twothr,fivthr,sq,rtmp
...
!
      implicit none
      integer momdim,m,n
      real(kind_phys)    qext, qsca, gqsc, pmom( 0:momdim, * )
      complex  sforw, sback, s1(*), s2(*), tforw(*), tback(*)
      logical  ok, wrong
!
      real(kind_phys)    accur, testqe, testqs, testgq, testpm( 0:1 )
...
!
        ...
        REAL(kind_phys), DIMENSION(nbin), INTENT(OUT) :: xnum_sect, xmas_sect
        real(kind_phys)&
        ...
        real(kind_phys) dlo_sect(nbin), dhi_sect(nbin)
!       real(kind_phys) erfc_num_recipes
        real(kind_phys) pi
...
!
      ...
      real(kind_phys) dp_wet_a,dp_core_a
      complex ri_shell_a,ri_core_a
      real(kind_phys) qextc,qscatc,qbackc,extc,scatc,backc,gscac
      real(kind_phys) vlambc
      integer n,kkk,jjj
        integer, save :: kcallmieaer
        data  kcallmieaer / 0 /
      real(kind_phys) pmom(0:7,1)
      real(kind_phys)weighte, weights, pscat
      real(kind_phys) pie,sizem
...
! /*---------------------------------------------------------------*/
      REAL(kind_phys) VLAMBc,RGcmin,RGcmax,RGc,SIGMAGc,SHELRc,SHELIc
      REAL(kind_phys) RINc,CORERc,COREIc
      INTEGER*4 NRGFLAGc,NANG
      REAL(kind_phys) QEXTc,QSCATc,QBACKc,EXTc,SCATc,BACKc,GSCAc
      REAL(kind_phys) ANGLESc(200),S1R(200),S1C(200),S2R(200),S2C(200)
      REAL(kind_phys) S11N,S11(200),S12(200),S33(200),S34(200),SPOL(200),SP(200)
      real(kind_phys) pmom(0:7,1)
      real(kind_phys) dp_wet_a,dp_core_a
      complex ri_shell_a,ri_core_a
!
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys)     RO, RFR, RFI, THETD(IT), QEXT, QSCAT, CTBRQS, &
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys)     T(5), TA(4), TB(2), TC(2), TD(2), TE(2), X, &
...
!     /*--------------------------------------------------------*/
      ...
      IMPLICIT REAL(kind_phys) (A-H, O-Z)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys) QEXTc,QSCATc,QBACKc,EXTc,SCATc,BACKc,GSCAc
      REAL(kind_phys) ANGLESc(*),S1R(*),S1C(*),S2R(*),S2C(*)
      REAL(kind_phys) S11N,S11(*),S12(*),S33(*),S34(*),SPOL(*),SP(*)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys) ALAMB, RGmin, RGmax, RGV, SIGMAG, RGCFRAC, RFRS,RFIS, RFRC, RFIC
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys) COSPHI(2*MXNANG-1), SCTPHS(2*MXNANG-1)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys) ALAMB, RGmin, RGmax, RGV, SIGMAG, RGCFRAC, RFRS, &
           RFIS, RFRC, RFIC
...
!     /*--------------------------------------------------------*/
      ...
      REAL(kind_phys)  THETA(MXNANG), ELTRMX(4,MXNANG,2), PII(3,MXNANG), &
             TAU(3,MXNANG), CSTHT(MXNANG),      SI2THT(MXNANG)

      REAL(kind_phys)   ROUT, RFRO, RFIO, DQEXT, DQSCAT, CTBRQS, DQBS, &
              RIN,  RFRI, RFII, WNUM
...
!
      ...
      real(kind_phys)    pmom( 0:momdim,1 )  ! the ",1" dimension is for historical reasons
      complex  a(500), b(500)
!
!           ** specification of local variables
!
...
!
      ...
      real(kind_phys)      am( 0:maxtrm ), bi( 0:mxmom2 ), bidel( 0:mxmom2 ), recip( maxrcp )
      complex cm( maxtrm ), dm( maxtrm ), cs( maxtrm ), ds( maxtrm )
      integer k,j,l,nummom,ld2,idel,m,i,mmax,imax
      real(kind_phys) sum
      logical    pass1, evenl
