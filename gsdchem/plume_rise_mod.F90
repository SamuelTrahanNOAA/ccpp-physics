module plume_rise_mod
  ...
  real(kind=kind_phys), parameter :: p1000 = 100000.  ! p at 1000mb (pascals)
...
!     integer, parameter :: nspecies=num_ebu
      real(kind=kind_phys), dimension (num_ebu) :: eburn_in 
      real(kind=kind_phys), dimension (kte,num_ebu) :: eburn_out
      real(kind=kind_phys), dimension (kte) :: u_in ,v_in ,w_in ,theta_in ,pi_in  &
                              ,rho_phyin ,qv_in ,zmid    &
                              ,z_lev
      real(kind=kind_phys), dimension(nveg_agreg) :: firesize,mean_fct
      real(kind=kind_phys) :: sum, ffirs, rcp,ratio,zk
!     real(kind=kind_phys),save,dimension(its:ite,jts:jte) :: ffirs
      ffirs=0.
      rcp=r_d/cp
      nspecies=num_ebu
