...
subroutine plumerise(m1,m2,m3,ia,iz,ja,jz,firesize,mean_fct   &
  ...
  real(kind=kind_phys)               :: burnt_area,dz_flam,rhodzi,dzi,frp
  real(kind=kind_phys)               :: q_smold_kgm2
  real(kind=kind_phys)               :: convert_smold_to_flam
...
real(kind=kind_phys),    intent(in) :: burnt_area, frp

! local variables
integer              :: i, icount, moist
real(kind=kind_phys)                 :: bfract, effload, heat, hinc, heat_fluxW
real(kind=kind_phys), dimension(2,4) :: heat_flux

! local parameters
integer, parameter :: use_last = 0
real(kind=kind_phys),    parameter :: beta = 0.88  !ref.: Paugam et al., 2015 FRP
...
real(kind=kind_phys) :: pea, btmp,etmp,vap1,vap2,gpkc,gpkh,gpki,deficit
...
!
real(kind=kind_phys), parameter :: TNULL=273.16, LAT=2.5008E6 &
                  ,EPSI=0.622 ,DB=1. ,NB=1500. !ALPHA=0.2 
real(kind=kind_phys) :: KA,KEINS,KZWEI,KDREI,VT
real(kind=kind_phys) :: A,B,C,D, CON,ACCRETE,total

real(kind=kind_phys) Y(6),ROH
