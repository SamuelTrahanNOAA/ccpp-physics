...
MODULE clm_lake
    ...
    public :: clm_lake_run, clm_lake_init, LAKEDEBUG, kind_lake

    ! The lake model internally uses "real(kind_lake)" for most
    ! calculations. That is kind_dbl_prec for now, to match what was
    ! done in WRF. We don't use kind_dbl_prec explicitly, anywhere
    ! else in this module. That allows us to easily try
    ! kind_lake=kind_phys in the future by changing only one line.
    integer, parameter :: kind_lake = kind_dbl_prec ! require double precision for most variables
   ...
   ! Initialize water type constants
    ...
    INTEGER  :: C,i,j,k
      ...
      real(kind_lake) :: lat_d(1)                ! latitude
      real(kind_lake) :: lon_d(1)                ! longitude

      real(kind_lake) :: qss_out             ! intermediate output to qss_water
        ...
        if_lake_is_here: if (flag_iter(i) .and. use_lake_model(i)/=0) THEN
           ...
           if(salty(i)/=0) then
             Tclim = tfrz + wght1*saltlk_T(num1)  &
                          + wght2*saltlk_T(num2)
             if(lakedebug) then
               print *,'Tclim,tsfc,t_lake3d',i,Tclim,tsfc_wat(i),t_lake3d(i,:),t_soisno3d(i,:)
             endif
             t_grnd2d(i) = min(Tclim+3.0_kind_lake,(max(real(tsfc_wat(i),kind_lake),Tclim-3.0_kind_lake)))
             do k = 1,nlevlake
               t_lake3d(i,k) = min(Tclim+3.0_kind_lake,(max(real(t_lake3d(i,k),kind_lake),Tclim-3.0_kind_lake)))
             enddo
             t_soisno3d(i,1) = min(Tclim+3.0_kind_lake,(max(real(t_soisno3d(i,1),kind_lake),Tclim-3.0_kind_phys)))
             if(lakedebug) then
               print *,'After Tclim,tsfc,t_lake3d',i,Tclim,tsfc_wat(i),t_lake3d(i,:),t_soisno3d(i,:)
             endif
           endif
	   ...
	   enddo
         ...
         enddo
            ...
            feedback: if(feedback_to_atmosphere) then
                ...
                discard2 = -9999
                discard3 = -9999
                call QSat(t_grnd(c),psfc,discard1,discard2,qss_out,discard3)
                qss_water(i) = qss_out
...
1001      continue ! Error handler, whether file was opened or not
    ...
    real(kind_lake),intent(in) :: xlat_d(1), xlon_d(1) ! grid location for debugging
...
SUBROUTINE ShalLakeFluxes(forc_t,forc_pbot,forc_psrf,forc_hgt,forc_hgt_q,           &  !i
  ...
  !==============================================================================
   ...
   ! implicit none
    ...
    real(kind_lake),intent(in) :: xlat_d(1),xlon_d(1)
    real(kind_lake),intent(in) :: forc_t(1)          ! atmospheric temperature (Kelvin)
   ...
   ! real(kind_lake),intent(in) :: forc_rho(1)        ! density (kg/m**3)
    ...
    real(kind_lake), parameter :: emg    = 0.97_kind_lake ! ground emissivity (0.97 for snow)
    ...
    do fc = 1, num_shlakec
       ...
       if (snl(c) < 0) then
           ...
           dzsur(c) = dz(c,jtop(c))/2._kind_lake
       else
           betaprime(c) = beta(islak)
           dzsur(c) = dz_lake(c,1)/2._kind_lake
       end if
...
1604     format('CLM_Lake ShalLakeFluxes: c=',I0,' sensible heat = ',F12.4,' latent heat =',F12.4, &
         ...
         print 1604, c, eflx_sh_tot(p), eflx_lh_tot(p), t_grnd(c), h2osno(c),xlat_d(1),xlon_d(1)
...
3018       format('CLM_Lake ShalLakeFluxes: WARNING: SH=',F12.4,' LH=',F12.4,' at xlat_d=',F10.3,' xlon_d=',F10.3)
           print 3018,eflx_sh_tot(p), eflx_lh_tot(p),xlat_d(1),xlon_d(1)
           unhappy = .true.
         end if
...
840        format('CLM_Lake ShalLakeFluxes: t_grnd is out of range: eflx_sh_tot(p)=',G20.12,' eflx_lh_tot(p)=',G20.12,' t_grnd(c)=',G20.12,' at p=',I0,' c=',I0,' xlat_d=',F10.3,' xlon_d=',F10.3)
           write(message,840) eflx_sh_tot(p),eflx_lh_tot(p),t_grnd(c),p,c,xlat_d(1),xlon_d(1)
           ! errmsg=message
           ! errflg=1
...
48              format('WARNING: At c=',I0,' level=',I0,' extreme t_soisno = ',F15.10)
       ...
       do fc = 1,num_shlakec
          ...
          if (j >= jtop(c)) then
             if (j < 1) then !snow layer
                ...
                zx(c,j) = zx(c,nlevlake) + dz_lake(c,nlevlake)/2._kind_lake + z(c,jprime)
                cvx(c,j) = cv(c,jprime)
          ...
          if (j >= jtop(c)) then
             if (j < 0) then !non-bottom snow layer
                ...
                tkix(c,j) = (tktopsoillay(c)*tk_lake(c,j)*dzp / &
                    (tktopsoillay(c)*dz_lake(c,j)/2._kind_lake + tk_lake(c,j)*z(c,1) ) )
  ...
  call SoilThermProp_Lake (snl,dz,zi,z,t_soisno,h2osoi_liq,h2osoi_ice,    &
    ...
    do fp = 1, num_shlakep
       ...
       if( (LAKEDEBUG .and. abs(errsoi(c)) < 1._kind_lake) &
