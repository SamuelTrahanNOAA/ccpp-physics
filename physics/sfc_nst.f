...
!     use_lake_model- logical, =T if lake model is used            im   !
...
!       flag(i) = wet(i) .and. .not.icy(i) .and. flag_iter(i)
        flag(i) = wet(i) .and. flag_iter(i) .and..not. use_lake_model(i)
...
!       if(wet(i) .and. .not.icy(i) .and. flag_guess(i)) then
        if(wet(i) .and. flag_guess(i) .and..not. use_lake_model(i)) then
          xt_old(i)      = xt(i)
          xs_old(i)      = xs(i)
          xu_old(i)      = xu(i)
          xv_old(i)      = xv(i)
...
!       if (wet(i) .and. .not.icy(i)) then
        if (wet(i) .and. .not. use_lake_model(i)) then
